...
public class Main {
    ...
    // ✅ NEW FUNCTION for Admin Logs
    public static void viewLogs() {
        String Query = "SELECT * FROM tbl_logs"; // Assuming you have a logs table
        String[] headers = {"Log ID", "User", "Action", "Date"};
        String[] columns = {"log_id", "u_name", "action", "log_date"};
        config conf = new config();
        conf.viewRecords(Query, headers, columns);
    }

    // ✅ NEW FUNCTION to simulate adding a log entry
    public static void addLog(String user, String action) {
        String sql = "INSERT INTO tbl_logs (u_name, action, log_date) VALUES (?, ?, datetime('now', 'localtime'))";
        config conf = new config();
        conf.addRecord(sql, user, action);
    }
                    ...
                    while (true) {
                        ...
                        if (result.isEmpty()) {
                            ...
                            if (stat.equals("Pending")) {
                                ...
                                addLog(uname, "Logged In"); // ✅ Log login activity
                                ...
                                if (type.equals("Admin")) {
                                    ...
                                    do {
                                        ...
                                        System.out.println("2. View Logs"); // ✅ new admin function
                                        ...
                                        switch (respo) {
                                            case 1:
                                                ...
                                                addLog(uname, "Approved user ID " + ids); // ✅ log admin action
                                                break;
